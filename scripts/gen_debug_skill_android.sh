...
Setup:
    ...
    export MINIS_DEBUG_TOKEN="$(adb shell run-as llc.slacker.eta cat files/debug_server_token)"
...
def call(host: str, method: str, params: dict, token: str | None) -> dict:
    if not token:
        raise SystemExit(
            ...
            "  adb shell run-as llc.slacker.eta cat files/debug_server_token\n"
...
```bash
adb forward tcp:5321 tcp:5321
TOK=$(adb shell run-as llc.slacker.eta cat files/debug_server_token)
