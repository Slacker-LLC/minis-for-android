...
class ToolPreflightTest {
    ...
    // ── [T-android-tool-arg-schema] schema conformance via preflight ──

    @Test
    fun `preflight rejects a value that violates the declared type`() {
        val numeric = AgentToolDefinition(
            name = "numeric_tool",
            description = "Takes a number",
            parameters = mapOf("count" to AgentToolParam("integer", "How many")),
        ...
        val rejection = ChatViewModel.preflightValidateToolCallImpl(
            "numeric_tool", JSONObject("""{"count":"many"}"""), listOf(numeric),
        )
        assertNotNull(rejection)
        assertTrue("got $rejection", rejection!!.contains("must be an integer"))
    }

    @Test
    fun `preflight rejects a value outside the declared enum`() {
        val modes = AgentToolDefinition(
            name = "mode_tool",
        ...
        assertNull(
            ChatViewModel.preflightValidateToolCallImpl(
                "mode_tool", JSONObject("""{"mode":"deep"}"""), listOf(modes),
            ),
        )
        val rejection = ChatViewModel.preflightValidateToolCallImpl(
            "mode_tool", JSONObject("""{"mode":"turbo"}"""), listOf(modes),
        )
        assertNotNull(rejection)
        assertTrue("got $rejection", rejection!!.contains("must be one of"))
    }

    @Test
    fun `preflight keeps reporting missing required fields before schema checks`() {
        // The pre-existing message must win when a field is simply absent, so
        // existing callers and tests keep their exact wording.
        val rejection = validate("file_edit", """{"path":"/a.txt"}""")
        assertNotNull(rejection)
        assertTrue("got $rejection", rejection!!.contains("missing required parameter"))
        assertTrue("got $rejection", rejection.contains("old_string"))
    }
}
