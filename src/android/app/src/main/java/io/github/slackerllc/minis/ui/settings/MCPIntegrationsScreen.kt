...
import io.github.slackerllc.minis.ui.components.DialogTextField
...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: io.github.slackerllc.minis.data.repository.EnvVarRepository? = null,
) {
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: io.github.slackerllc.minis.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<io.github.slackerllc.minis.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                io.github.slackerllc.minis.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                io.github.slackerllc.minis.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): io.github.slackerllc.minis.mcp.oauth.MCPOAuthConfig? {
        val cfg = io.github.slackerllc.minis.mcp.oauth.MCPOAuthConfig(
                    ...
                    if (oauthAuthorized) {
                        MinisTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    io.github.slackerllc.minis.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    io.github.slackerllc.minis.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = io.github.slackerllc.minis.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is io.github.slackerllc.minis.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is io.github.slackerllc.minis.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is io.github.slackerllc.minis.mcp.oauth.MCPOAuthController.Result.Failed -> {
