...
class DebugRPCHandler(private val context: Context) {
    ...
    private fun handleAppInfo(): JSONObject {
        ...
        return JSONObject().apply {
            ...
            val ubuntuRunning = io.github.slackerllc.minis.sandbox.ubuntu.UbuntuRuntime.snapshot.value.running
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = io.github.slackerllc.minis.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            io.github.slackerllc.minis.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private fun handleMcpStatus(): JSONObject {
        val s = io.github.slackerllc.minis.mcp.server.MCPServerManager.status()
        return JSONObject()
    ...
    private fun handleMcpStart(): JSONObject {
        val ok = io.github.slackerllc.minis.mcp.server.MCPServerManager.start()
        return JSONObject().put("started", ok)
    }

    private fun handleMcpStop(): JSONObject {
        io.github.slackerllc.minis.mcp.server.MCPServerManager.stop()
    ...
    private fun handleMcpSetToken(params: JSONObject): JSONObject {
        ...
        io.github.slackerllc.minis.mcp.server.TokenStore.save(
            listOf(io.github.slackerllc.minis.mcp.server.TokenStore.Token(id = id, token = token, scope = scope)),
        )
        io.github.slackerllc.minis.mcp.server.MCPServerManager.init(context)
        return JSONObject().put("saved", true).put("configured", io.github.slackerllc.minis.mcp.server.TokenStore.isConfigured)
    }
    ...
    private fun handleMcpConfirmAnswer(params: JSONObject): JSONObject {
        ...
        val result = io.github.slackerllc.minis.mcp.server.MCPServerManager.debugApproveConfirm(id, method)
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = io.github.slackerllc.minis.data.UpdateChecker.check()) {
            is io.github.slackerllc.minis.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            io.github.slackerllc.minis.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            io.github.slackerllc.minis.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is io.github.slackerllc.minis.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            io.github.slackerllc.minis.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            io.github.slackerllc.minis.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is io.github.slackerllc.minis.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = io.github.slackerllc.minis.data.UpdateChecker.download(context, url)) {
            is io.github.slackerllc.minis.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is io.github.slackerllc.minis.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = io.github.slackerllc.minis.data.UpdateChecker.canInstall(context)
        return try {
            io.github.slackerllc.minis.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = MinisKernel.resolveHostPath(path) ?: run {
            val rootfsDir = io.github.slackerllc.minis.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in io.github.slackerllc.minis.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", io.github.slackerllc.minis.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [io.github.slackerllc.minis.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = io.github.slackerllc.minis.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = io.github.slackerllc.minis.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [io.github.slackerllc.minis.sandbox.offload.ModelUseOffloadHandler]
    ...
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = io.github.slackerllc.minis.sandbox.MinisKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as io.github.slackerllc.minis.MinisApp
        val handler = io.github.slackerllc.minis.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = io.github.slackerllc.minis.sandbox.NativeOffloadRequest(
            pid = -1,
     ...
     * [io.github.slackerllc.minis.sandbox.offload.SessionsOffloadHandler] for e2e
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as io.github.slackerllc.minis.MinisApp
        val handler = io.github.slackerllc.minis.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = io.github.slackerllc.minis.sandbox.NativeOffloadRequest(
            pid = -1,
        ...
        return when (sub) {
            "set" -> {
                ...
                kotlinx.coroutines.runBlocking {
                    io.github.slackerllc.minis.config.ConfigBridge.performWriteBatch(
                        items = items,
            ...
            "get" -> {
                ...
                io.github.slackerllc.minis.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", io.github.slackerllc.minis.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
                ...
                val known = io.github.slackerllc.minis.config.ConfigBridge.allTopics()
                    .let { arr -> (0 until arr.length()).any { arr.optString(it) == topic } }
                if (!known) {
                    throw RPCException(-32602, "Unknown topic '$topic' — call subcommand=topics")
                }
                val fields = io.github.slackerllc.minis.config.ConfigBridge.fieldsForTopic(topic)
                JSONObject().apply {
                    put("ok", true)
                    put("topic", topic)
            ...
            "audit-list" -> {
                ...
                io.github.slackerllc.minis.config.ConfigBridge.auditList(limit, scope)
            }
