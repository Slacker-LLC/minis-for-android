...
import io.github.slackerllc.minis.tools.JobRegistry
import io.github.slackerllc.minis.tools.SubagentLimits
...
internal object AgentRpcMethods {
    ...
    fun sessionPermissionGet(context: Context, params: JSONObject): JSONObject {
        ...
        return JSONObject().apply {
            ...
            io.github.slackerllc.minis.tools.SessionPermissionStore.preset(context, sessionId)
                ?.let { put("preset", it) }
    ...
    fun sessionPermissionSet(context: Context, params: JSONObject): JSONObject {
        ...
        if (preset != null && !io.github.slackerllc.minis.tools.SessionPermissionStore.isKnownPreset(preset)) {
            throw RPCException(-32602, "preset must be workspace-write | danger-full-access | null")
        }
        io.github.slackerllc.minis.tools.SessionPermissionStore.setPreset(context, sessionId, preset)
        // Same three events the /permission command emits, so the DSH
        // permissions projection (App and Web) sees one state.
        val effective = preset ?: io.github.slackerllc.minis.tools.SessionPermissionStore.WORKSPACE_WRITE
        val sandboxMode = if (effective == io.github.slackerllc.minis.tools.SessionPermissionStore.DANGER_FULL_ACCESS)
            "danger-full-access" else "workspace-write"
        val approvalPolicy = if (effective == io.github.slackerllc.minis.tools.SessionPermissionStore.DANGER_FULL_ACCESS)
            "never" else "ask"
        io.github.slackerllc.minis.ui.chat.SessionEventHub.append(sessionId, "permission/preset", JSONObject().put("preset", effective))
        io.github.slackerllc.minis.ui.chat.SessionEventHub.append(sessionId, "sandbox/mode", JSONObject().put("mode", sandboxMode))
        io.github.slackerllc.minis.ui.chat.SessionEventHub.append(sessionId, "approval/policy", JSONObject().put("policy", approvalPolicy))
        return JSONObject().apply {
            put("sessionId", sessionId)
    ...
    fun approvalsList(context: Context, params: JSONObject): JSONObject {
        ...
        val arr = JSONArray()
        for (r in io.github.slackerllc.minis.tools.ApprovalSeam.pendingFor(sessionId)) {
    ...
    fun approvalsAnswer(context: Context, params: JSONObject): JSONObject {
        ...
        val ok = io.github.slackerllc.minis.tools.ApprovalSeam.answer(id, params.optBoolean("allowed", false))
