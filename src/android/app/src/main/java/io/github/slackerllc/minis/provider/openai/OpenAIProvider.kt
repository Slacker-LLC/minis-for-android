package io.github.slackerllc.minis.provider.openai
...
import io.github.slackerllc.minis.data.model.LLMMediaAttachment
import io.github.slackerllc.minis.data.model.LLMMessage
import io.github.slackerllc.minis.data.model.LLMModel
import io.github.slackerllc.minis.data.model.LLMResponse
...
import io.github.slackerllc.minis.provider.thinking.ThinkingResolveContext
...
import io.github.slackerllc.minis.provider.applyUserAgentOverride
...
import io.github.slackerllc.minis.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [io.github.slackerllc.minis.provider.thinking.ThinkingRuleResolver]'s
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(io.github.slackerllc.minis.network.NetworkMonitor.sharedLLMConnectionPool)
        .eventListenerFactory { OkHttpNetTraceListener() }
        .let { io.github.slackerllc.minis.provider.ProviderTransportPolicy.configureClient(it, basePath) }
        .build()
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = io.github.slackerllc.minis.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            io.github.slackerllc.minis.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= io.github.slackerllc.minis.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            io.github.slackerllc.minis.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            io.github.slackerllc.minis.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (io.github.slackerllc.minis.BuildConfig.DEBUG) {
                io.github.slackerllc.minis.debug.LLMRequestLog.add(
                    io.github.slackerllc.minis.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            io.github.slackerllc.minis.logging.AppLogger.debug(
                                "OpenAIProvider",
        ...
        for (i in 0 until dataArray.length()) {
            ...
            if (b64.isNotEmpty()) {
                ...
                if (urlStr.isNotEmpty()) {
                    try {
                        val safeUrl = io.github.slackerllc.minis.provider.ProviderTransportPolicy
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = io.github.slackerllc.minis.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = io.github.slackerllc.minis.provider.ImageBudget.compressUnderBudget(data)
    ...
    private fun buildResponsesAPIBody(
        ...
        if (io.github.slackerllc.minis.data.FastModePrefs.isEnabled() &&
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> io.github.slackerllc.minis.logging.AppLogger.error(
                                        "OpenAIProvider",
