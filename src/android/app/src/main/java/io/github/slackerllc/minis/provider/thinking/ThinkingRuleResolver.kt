package io.github.slackerllc.minis.provider.thinking
...
object ThinkingRuleResolver {
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = io.github.slackerllc.minis.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to io.github.slackerllc.minis.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = io.github.slackerllc.minis.provider.anthropic.AnthropicProvider
