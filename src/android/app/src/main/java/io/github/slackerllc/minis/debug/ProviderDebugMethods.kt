...
internal object ProviderDebugMethods {
            ...
            if (includeMembers) {
                ...
                for (memberId in group.memberEntryIds) {
                    ...
                    val entry = cfg.modelEntries.find { it.id == memberId }
                        ?: io.github.slackerllc.minis.data.model.SystemVoiceEntries.resolve(memberId)
                        ?: continue
    ...
    suspend fun quickTest(context: Context, params: JSONObject): JSONObject {
        ...
        val kinds: List<io.github.slackerllc.minis.ui.components.QuickTestKind> = if (requested == null) {
            io.github.slackerllc.minis.ui.components.applicableKinds(entry)
        } else {
            (0 until requested.length()).mapNotNull { i ->
                when (requested.optString(i).lowercase()) {
                    "text" -> io.github.slackerllc.minis.ui.components.QuickTestKind.TEXT
                    "speechout", "speech_out", "tts" -> io.github.slackerllc.minis.ui.components.QuickTestKind.SPEECH_OUT
                    "transcription", "asr" -> io.github.slackerllc.minis.ui.components.QuickTestKind.TRANSCRIPTION
                    "imagegen", "image_gen", "image" -> io.github.slackerllc.minis.ui.components.QuickTestKind.IMAGE_GEN
                    else -> null
                }
            }
        }
        ...
        for (kind in kinds) {
            ...
            val state = try {
                io.github.slackerllc.minis.ui.components.performTest(kind, entry, repo, context)
            } catch (e: Exception) {
                io.github.slackerllc.minis.ui.components.QuickTestState.Failure(
            ...
            results.put(JSONObject().apply {
                ...
                when (state) {
                    is io.github.slackerllc.minis.ui.components.QuickTestState.TextReply -> {
                        put("status", "ok"); put("detail", state.text.take(400))
                    }
                    is io.github.slackerllc.minis.ui.components.QuickTestState.AudioReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "audio ${state.data.size} bytes")
                    }
                    is io.github.slackerllc.minis.ui.components.QuickTestState.ImageReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "image ${state.data.size} bytes")
                    }
                    is io.github.slackerllc.minis.ui.components.QuickTestState.Failure -> {
