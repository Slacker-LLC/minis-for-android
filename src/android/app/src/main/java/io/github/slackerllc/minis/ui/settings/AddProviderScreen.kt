...
import io.github.slackerllc.minis.auth.OpenAIOAuthManager
import io.github.slackerllc.minis.auth.OpenRouterOAuthManager
...
import io.github.slackerllc.minis.ui.components.RowLabel
...
fun AddProviderScreen(
    ...
    var selectedVoiceTemplate by remember {
        mutableStateOf<io.github.slackerllc.minis.data.model.VoiceProviderTemplate?>(null)
    }
...
private fun ChooseProviderScreen(
    ...
    onSelectVoiceTemplate: (io.github.slackerllc.minis.data.model.VoiceProviderTemplate) -> Unit = {},
) {
    SettingsScaffold(
        ...
        val templates = io.github.slackerllc.minis.data.model.VoiceProviderTemplate.all
        ...
        SettingsSection(
            ...
            templates.forEachIndexed { index, template ->
                val capabilityRes = when (template.capability) {
                    io.github.slackerllc.minis.data.model.VoiceProviderTemplate.Capability.TTS ->
                        R.string.add_provider_voice_capability_tts
                    io.github.slackerllc.minis.data.model.VoiceProviderTemplate.Capability.ASR ->
                        R.string.add_provider_voice_capability_asr
                    io.github.slackerllc.minis.data.model.VoiceProviderTemplate.Capability.BOTH ->
...
private fun ConfigureProviderScreen(
    ...
    providerRepository: ProviderRepository,
    voiceTemplate: io.github.slackerllc.minis.data.model.VoiceProviderTemplate? = null,
    onBack: () -> Unit,
    onSaved: () -> Unit,
) {
...
private fun ColumnScope.OAuthConfigSection(
    ...
    val manualHttpBlocked = io.github.slackerllc.minis.provider.ProviderTransportPolicy
                            ...
                            try {
                                when (providerType) {
                                    ProviderType.kimiCode -> {
                                        ...
                                        val key = io.github.slackerllc.minis.auth.KimiOAuthManager.login(
                                    ...
                                    ProviderType.anthropic -> {
                                        val key = io.github.slackerllc.minis.auth.ClaudeOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    ProviderType.openAI -> {
                                    ...
                                    ProviderType.xAI -> {
                                        val key = io.github.slackerllc.minis.auth.XAIOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    else -> {
                                ...
                                // signal.
                                errorMessage = if (e is io.github.slackerllc.minis.auth.OAuthNetworkUnreachableException) {
