...
import io.github.slackerllc.minis.provider.anthropic.AnthropicProvider
import io.github.slackerllc.minis.provider.gemini.GeminiProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    io.github.slackerllc.minis.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw io.github.slackerllc.minis.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = io.github.slackerllc.minis.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw io.github.slackerllc.minis.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
                    )
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${io.github.slackerllc.minis.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    io.github.slackerllc.minis.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = io.github.slackerllc.minis.auth.KimiOAuthManager(context, instance.id)
