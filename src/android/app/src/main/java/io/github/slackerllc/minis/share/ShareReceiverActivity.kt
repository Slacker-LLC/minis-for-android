package io.github.slackerllc.minis.share
...
class ShareReceiverActivity : ComponentActivity() {
    ...
    private fun promptProviderImportOrAttach(
        ...
        return try {
            android.app.AlertDialog.Builder(this)
                .setTitle(getString(io.github.slackerllc.minis.R.string.share_provider_json_title))
                .setMessage(getString(io.github.slackerllc.minis.R.string.share_provider_json_message))
                .setPositiveButton(getString(io.github.slackerllc.minis.R.string.share_provider_json_import)) { _, _ ->
                ...
                .setNegativeButton(getString(io.github.slackerllc.minis.R.string.share_provider_json_attach)) { _, _ ->
                    finishWithAttachmentFlow(items)
                }
                .setOnCancelListener {
    ...
    private fun importProviderJson(json: String) {
        ...
        val repo = (applicationContext as? io.github.slackerllc.minis.MinisApp)?.providerRepositoryOrNull
        if (repo == null) {
            AppLogger.warning(TAG, "providerRepository unavailable; cannot import")
            toast(getString(io.github.slackerllc.minis.R.string.share_provider_json_import_failed))
            return
        }
        val label = repo.importInstanceJSON(json)
        if (label != null) {
            AppLogger.info(TAG, "imported provider \"$label\" from shared JSON")
            toast(getString(io.github.slackerllc.minis.R.string.share_provider_json_imported, label))
        } else {
    ...
    private fun launchMainActivity() {
        ...
        when (outcome) {
            ...
            ShareHandoffPolicy.Outcome.FAILED -> {
                ...
                try {
                    toast(getString(io.github.slackerllc.minis.R.string.share_open_app_failed))
                } catch (t: Throwable) {
    ...
    private fun mainActivityIntent(flags: Int): Intent =
        Intent(this, Class.forName("io.github.slackerllc.minis.MainActivity")).apply {
            addFlags(flags)
