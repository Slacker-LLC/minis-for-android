package io.github.slackerllc.minis.config

import android.content.Context
import io.github.slackerllc.minis.config.collections.EnvVarsCollection
import io.github.slackerllc.minis.service.DynamicIslandSupport
import io.github.slackerllc.minis.config.collections.GroupsCollection
import io.github.slackerllc.minis.config.collections.ModelsCollection
import io.github.slackerllc.minis.config.collections.ProvidersCollection
import io.github.slackerllc.minis.config.fields.ClosureField
import io.github.slackerllc.minis.config.fields.PrefsBoolField
import io.github.slackerllc.minis.config.fields.PrefsDoubleField
import io.github.slackerllc.minis.config.fields.PrefsEnumField
import io.github.slackerllc.minis.config.fields.PrefsIntCodedEnumField
import io.github.slackerllc.minis.config.fields.PrefsIntField
import io.github.slackerllc.minis.config.fields.PrefsLongField
import io.github.slackerllc.minis.config.fields.PrefsStringField
import io.github.slackerllc.minis.config.fields.ReadOnlyField
...
import io.github.slackerllc.minis.ui.chat.ChatViewModelStore
...
internal object ConfigBuiltins {
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = io.github.slackerllc.minis.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        val appearancePrefs = context.getSharedPreferences(
            io.github.slackerllc.minis.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsBoolField(
                ...
                key = io.github.slackerllc.minis.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = io.github.slackerllc.minis.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = io.github.slackerllc.minis.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = io.github.slackerllc.minis.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(io.github.slackerllc.minis.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    io.github.slackerllc.minis.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(io.github.slackerllc.minis.config.collections.ThinkingRulesCollection(providerRepo))
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, io.github.slackerllc.minis.data.model.ProviderInstance>(cfg.instances.size)
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(io.github.slackerllc.minis.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    io.github.slackerllc.minis.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, io.github.slackerllc.minis.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, io.github.slackerllc.minis.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [io.github.slackerllc.minis.agent.SoulStore.isOverLimit].
        ...
        fun loadCurrent(): io.github.slackerllc.minis.agent.SoulFile =
            io.github.slackerllc.minis.agent.SoulStore.load(context)
                ?: io.github.slackerllc.minis.agent.SoulMDParser.parse(
                    io.github.slackerllc.minis.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: io.github.slackerllc.minis.agent.SoulFile) {
            io.github.slackerllc.minis.agent.SoulStore.save(context, file)
        }
            ...
            ClosureField(
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        io.github.slackerllc.minis.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        io.github.slackerllc.minis.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${io.github.slackerllc.minis.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${io.github.slackerllc.minis.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (io.github.slackerllc.minis.agent.SystemPromptBuilder
                    ...
                    val check = io.github.slackerllc.minis.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is io.github.slackerllc.minis.agent.SoulBodyLimitCheck.Ok -> Unit
                        is io.github.slackerllc.minis.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${io.github.slackerllc.minis.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is io.github.slackerllc.minis.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${io.github.slackerllc.minis.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()
