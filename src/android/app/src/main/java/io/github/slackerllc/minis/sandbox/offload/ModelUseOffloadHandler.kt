package io.github.slackerllc.minis.sandbox.offload
...
import io.github.slackerllc.minis.provider.safeOptString
import io.github.slackerllc.minis.sandbox.NativeOffloadHandler
import io.github.slackerllc.minis.sandbox.NativeOffloadRequest
import io.github.slackerllc.minis.sandbox.NativeOffloadResult
...
class ModelUseOffloadHandler(
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        ...
        // the shell's cwd, so MinisKernel.resolveHostPath would silently drop a
        ...
        if (hasAudioInput && provider !is io.github.slackerllc.minis.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? io.github.slackerllc.minis.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? io.github.slackerllc.minis.provider.openai.OpenAIProvider)?.let { openAI ->
        ...
        val response = try {
            runBlocking {
                io.github.slackerllc.minis.provider.LLMRetryPolicy.withRetry {
        ...
        if (outputPath != null) {
            ...
            val hostFile = sessionScopedHostFile(outputPath, sessionId)
                ?: MinisKernel.resolveHostPath(outputPath)
                ?: return NativeOffloadResult(
                    2,
            ...
            val attachDir = sessionScopedHostFile("/var/minis/attachments", sessionId)
                ?: MinisKernel.resolveHostPath("/var/minis/attachments")
            if (attachDir != null) {
                attachDir.mkdirs()
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: io.github.slackerllc.minis.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> io.github.slackerllc.minis.data.model.ImageEndpointMode.auto
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: io.github.slackerllc.minis.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        ...
        provider: io.github.slackerllc.minis.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == io.github.slackerllc.minis.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> io.github.slackerllc.minis.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == io.github.slackerllc.minis.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == io.github.slackerllc.minis.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == io.github.slackerllc.minis.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, io.github.slackerllc.minis.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is io.github.slackerllc.minis.data.model.LLMError.ProviderError ||
                e is io.github.slackerllc.minis.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, io.github.slackerllc.minis.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: io.github.slackerllc.minis.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
     ...
     * global (last-writer-wins) MinisKernel.bindMounts map. That global map is
     * overwritten by ExecutionCoordinator.buildSessionBindMounts on every shell
     * build, so MinisKernel.resolveHostPath("/var/minis/attachments") returns
    ...
    private fun logModelUseWrite(responsePath: String, hostFile: File, sessionId: String?) {
        Log.i(
            ...
            "[ModelUseWrite] path=$responsePath hostFile=${hostFile.absolutePath} " +
                "sessionId=$sessionId globalAttachMount=${MinisKernel.bindMounts["/var/minis/attachments"]}",
        )
    }
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        io.github.slackerllc.minis.offload.ModelUseManager.entryDict(
            entry,
     ...
     *    [MinisKernel.resolveHostPath] (which already handles
    ...
    private fun resolveImageUrl(url: String): LLMMessage.ImagePart {
        ...
        val hostFile: File = MinisKernel.resolveHostPath(linuxPath)
