...
/**
 * [T-eta-xposed-groups] Fixed queries over an already-opened read-only database; the app-side
 * snapshot path and the hook inside the ColorOS memory process share this one implementation.
 *
 * Ported from Eta `agent/tool/ColorOsMemoryDatabaseQuery.kt` (Mangi-11/Eta @ c15de97); attribution
 * in THIRD_PARTY_LICENSES.md. Every table and column is looked up defensively and every string that
 * reaches a statement is bounded, because the schema belongs to another vendor's app and moves
 * between releases.
 */
