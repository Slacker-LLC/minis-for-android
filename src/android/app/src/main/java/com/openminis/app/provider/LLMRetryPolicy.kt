 ...
 *
 * [C6-android-model-failure-discipline] The verdict itself comes from
 * [LLMFailureClassifier] - the same classifier the agent loop's auto-retry
 * uses (ported from Eta agent/model/AgentModelFailure.kt) - so the two retry
 * decision points cannot drift apart. One consequence of the port: TLS and
 * protocol failures wrapped in a NetworkError are no longer retried (Eta's
 * transport() leaves them unclassified), while a request timeout still is.
 */
object LLMRetryPolicy {
    ...
    fun isRetryable(t: Throwable): Boolean = LLMFailureClassifier.isRetryable(t)
