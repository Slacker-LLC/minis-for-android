...
class BackupImporter(
    ...
    /**
     * [T-android-skill-backup-restore-transaction] Restore the skills category
     * through the skill install transaction.
     *
     * This used to hand the package's `skills/<id>/…` tree to
     * [BackupRestoreFiles], which wrote it straight into
     * `<filesDir>/minis-global/skills` — the directory and the registry the
     * install transaction owns — with no lock, no journal and no budget. A restore
     * could therefore interleave with an install on the same tree, and a write
     * killed half way left a skill no recovery knew about, because nothing had
     * recorded what the whole skill was supposed to be. Every skill now goes
     * through [SkillRepository.restoreFromBackup]: staged below the skills root,
     * committed with an atomic move, rolled back to the installed version when
     * anything fails, under the same cross-process lock as every other skill write.
     */
    private suspend fun importSkills(root: File, fileIndex: List<BackupFileIndexEntry>): CategoryReport {
        val report = CategoryReport(BackupCategory.SKILLS.key)
        val repository = app?.let { if (it.subsystemsReady()) it.skillRepository else null } ?: run {
            // The transaction owns the skill files and their registry row together.
            // A restore that cannot reach it must not write either one directly.
            report.failed = "skill repository unavailable"
            return report
        }

        // One transaction per skill, in package order: a skill is what the
        // transaction commits, and one unusable skill must not stop the others.
        for ((skillId, entries) in groupSkillEntries(fileIndex, report)) {
            val payload = readSkillPayload(root, skillId, entries, report)
            if (payload == null) {
                // Nothing was written. A payload over the transaction's budget, or
                // one carrying an unusable path, is refused as a whole rather than
                // committed in part.
                report.unreadable += 1
                continue
            }
            when (val outcome = repository.restoreFromBackup(skillId, payload.files)) {
                is SkillRepository.BackupSkillRestore.Restored -> {
                    // The manifest counts skills, not files, so these two counters
                    // are what the restore report (and its short-restore warning)
                    // compares against the declared skill count.
                    if (outcome.created) report.imported += 1 else report.updated += 1
                    report.filesWritten += payload.files.size
                    report.bytesWritten += payload.bytes
                    AppLogger.info(
                        TAG,
                        "[Restore] skills/" + skillId + ": " +
                            (if (outcome.created) "imported" else "updated") +
                            " " + payload.files.size + " file(s), " + payload.bytes + " byte(s)",
                    )
                }
                is SkillRepository.BackupSkillRestore.Refused -> {
                    if (outcome.recoveryRequired) {
                        // Fail-closed: an unfinished transaction refuses every
                        // following skill write too, so the category stops with a
                        // message the report shows instead of counting the rest as
                        // restored.
                        report.failed = "An unfinished skill recovery has to finish first. " +
                            "Open the Skills screen, then restore again. (" + outcome.reason + ")"
                        AppLogger.error(
                            TAG,
                            "[Restore] skills refused, recovery required: " + outcome.reason,
                        )
                        return report
                    }
                    report.unreadable += 1
                    AppLogger.warning(TAG, "[Restore] skills/" + skillId + " refused: " + outcome.reason)
                }
            }
        }
        return report
    ...
    /**
     * Group the package's skill entries by skill id, in package order.
     *
     * An entry outside `skills/<id>/…`, or one whose id is not a usable directory
     * name, is refused here: the transaction refuses the same id, and counting it
     * as a rejected path keeps it out of the per-skill payloads, where it would
     * only show up as an unreadable skill.
     */
    private fun groupSkillEntries(
        fileIndex: List<BackupFileIndexEntry>,
        report: CategoryReport,
    ): Map<String, MutableList<BackupFileIndexEntry>> {
        ...
        for (entry in fileIndex) {
            ...
            grouped.getOrPut(skillId) { mutableListOf() }.add(entry)
        }
        return grouped
    }

    /** `skills/<id>/…` → `<id>`, or null when the entry does not name a usable skill. */
    private fun skillIdOfPath(path: String): String? {
        if (!path.startsWith("skills/")) return null
        val id = path.removePrefix("skills/").substringBefore('/')
        return id.takeIf { isSafeSkillTransactionId(it) }
    }

    /** One skill's staged payload: what the transaction commits, and its size. */
    private class SkillPayload(
        val files: List<SkillRepository.BackupSkillFile>,
        val bytes: Long,
    )

    /**
     * Read one skill out of the package: blob bytes for its files, tombstones
     * counted but never written, and the transaction's own entry and byte budgets
     * applied BEFORE the payload is materialised, so a package cannot make the
     * importer hold more than the transaction would accept.
     *
     * Returns null when the skill cannot be restored as a whole. The reason is
     * logged here; the caller only adds the count, because the counters are what
     * the restore report shows.
     */
    private fun readSkillPayload(
        root: File,
        skillId: String,
        entries: List<BackupFileIndexEntry>,
        report: CategoryReport,
    ): SkillPayload? {
        val prefix = "skills/" + skillId + "/"
        val files = mutableListOf<SkillRepository.BackupSkillFile>()
        var bytes = 0L
        for (entry in entries) {
            val tombstone = entry.skipped
            if (tombstone != null) {
                // Tombstones carry no content by design (§3.4): counted, never
                // written. Under the transaction's PRESERVE base the local copy of
                // that path stays, which is what the direct write did too.
                if (tombstone == "not_downloaded") report.notDownloadedInPackage += 1
                else report.sizeSkippedInPackage += 1
                continue
            }
            // An empty directory has no representation in the staged tree: the
            // transaction commits files, and a skill is anchored by its SKILL.md.
            if (entry.isDirectory == true) continue

            val sha = entry.sha256 ?: continue
            val blob = File(File(File(root, "blobs"), sha.take(2)), sha)
            if (!blob.isFile) {
                // Same rule as the file-tree restore: content the index lists but
                // the package does not carry is counted, surfaces in the restore
                // warnings, and leaves the local copy of that path alone.
                report.missingBlobs += 1
                continue
            }
            val relativePath = entry.path.removePrefix(prefix)
            if (relativePath.isEmpty() || relativePath.endsWith("/")) {
                AppLogger.error(TAG, "[Restore] skills/" + skillId + ": entry names a directory: " + entry.path)
                return null
            }
            if (files.size >= SkillArchiveReader.MAX_ENTRIES) {
                AppLogger.error(
                    TAG,
                    "[Restore] skills/" + skillId + ": more than " + SkillArchiveReader.MAX_ENTRIES + " entries",
                )
                return null
            }
            if (blob.length() > SkillArchiveReader.MAX_ENTRY_BYTES ||
                bytes + blob.length() > SkillArchiveReader.MAX_TOTAL_BYTES
            ) {
                AppLogger.error(
                    TAG,
                    "[Restore] skills/" + skillId + ": payload is larger than the skill budgets " +
                        "(entry " + SkillArchiveReader.MAX_ENTRY_BYTES + ", total " +
                        SkillArchiveReader.MAX_TOTAL_BYTES + " bytes)",
                )
                return null
            }
            val content = try {
                ...
                AppLogger.error(TAG, "[Restore] skills/" + skillId + ": " + entry.path + " unreadable: " + error.message)
                report.missingBlobs += 1
                continue
            }
            files += SkillRepository.BackupSkillFile(relativePath, content)
            bytes += content.size
        }
        return SkillPayload(files, bytes)
    }
