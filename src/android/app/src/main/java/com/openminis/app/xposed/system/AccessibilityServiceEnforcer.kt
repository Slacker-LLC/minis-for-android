...
/**
 * [T-eta-xposed-groups] Keeping this app's accessibility service usable for as long as
 * system_server lives, according to the user's choice.
 *
 * Ported from Eta `hook/system/AccessibilityServiceEnforcer.kt` (Mangi-11/Eta @ c15de97);
 * attribution in THIRD_PARTY_LICENSES.md. Protection is off by default; once it is on, the enforcer
 * maintains only this app's component and the master switch in the owner user, and always keeps
 * every other accessibility service exactly as it was. All work is posted to Android's own
 * background thread: no thread of its own, no timer, no polling loop.
 *
 * The service list is edited through the same setting the platform writes, so a repair is a
 * read-modify-write of the current value - which is what the pure helpers at the bottom of this
 * file decide, and what the unit tests pin down.
 */
internal class AccessibilityServiceEnforcer(
    ...
    private fun ensureActiveObserversRegistered(context: Context) {
        ...
        if (!packageReceiverRegistered) {
            ...
            try {
                ...
                logFailure("无法监听 Minis 包变化", failure)
            }
        }
    ...
    private fun unregisterActiveObservers(context: Context) {
        ...
        if (packageReceiverRegistered) {
            try {
                ...
                logFailure("无法注销 Minis 包监听", failure)
            }
        }
        if (lifecycleReceiverRegistered) {
            try {
    ...
    private fun completeServiceRepair(
        ...
        try {
            ...
            logFailure("无法重新启用 Minis 无障碍服务", failure)
        } finally {
            repairInProgress.set(false)
        }
        if (isEnforcementEnabled(context)) {
    ...
    private fun isExpectedServiceValid(context: Context): Boolean {
        ...
        val serviceInfo = try {
            ...
            logFailure("无法校验 Minis 无障碍服务组件", failure)
            return false
        }
