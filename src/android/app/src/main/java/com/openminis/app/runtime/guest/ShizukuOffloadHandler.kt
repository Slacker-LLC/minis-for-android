...
import com.openminis.app.tools.android.AndroidDisplaySizeParser
import com.openminis.app.tools.android.FocusedWindowParser
...
class ShizukuOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun activityTop(args: OffloadArgs): NativeOffloadResult {
        ...
        if (m != null) {
            return okEnvelope(JSONObject()
                ...
                .put("component", "${m.groupValues[1]}/${m.groupValues[2]}")
                .put("source", "activity"), args)
        }
        ...
        val obj = if (focused != null) {
            JSONObject()
                .put("packageName", focused.packageName)
                .put("activityName", focused.component.substringAfter('/', ""))
                .put("component", focused.component)
                .put("source", "window")
                .put("rawLine", focused.rawLine)
        } else {
            JSONObject().put("packageName", "unknown").put("activityName", "unknown").put("source", "unknown")
        }
    ...
    private fun displayList(args: OffloadArgs): NativeOffloadResult {
        ...
        return okEnvelope(JSONObject()
            ...
            .put("logicalWidth", logical?.first ?: -1)
            .put("logicalHeight", logical?.second ?: -1)
            .put("sizeOverridden", override != null), args)
    }
