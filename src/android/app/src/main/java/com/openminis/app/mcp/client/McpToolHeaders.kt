...
/**
 * Ported from Eta `agent/mcp/McpToolHeaders.kt` (Mangi-11/Eta @ c15de97); attribution in
 * THIRD_PARTY_LICENSES.md.
 *
 * Maps a tool argument to a request header when the server's schema says so with the
 * modern MCP extension `x-mcp-header`: the parameter travels as `Mcp-Param-<name>` on
 * the `tools/call` request instead of inside the JSON body, which is how a server asks
 * for a secret to be passed out of band. This repository did not read the extension at
 * all, so a server that requires it silently received nothing.
 */
...
/**
 * Header values must stay in the printable-ASCII range (or use the spec's base64
 * wrapper); anything else — including a value with meaningful leading/trailing space, or
 * one that already looks like the wrapper — is encoded so the server can tell the two
 * apart.
 */
