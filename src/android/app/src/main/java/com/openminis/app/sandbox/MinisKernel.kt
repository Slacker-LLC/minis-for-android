...
object MinisKernel {
    ...
    val bindMounts: MutableMap<String, String>
        get() = com.openminis.app.sandbox.ubuntu.UbuntuPaths.bindMounts

    /**
     * Seed mount registry + SAF snapshot. Idempotent; called from
     * MinisApp.onCreate. Ubuntu runtime start is handled by
     * [com.openminis.app.sandbox.ubuntu.UbuntuRuntime.ensureReady].
     */
    ...
    fun resolveHostPath(linuxPath: String): File? =
        com.openminis.app.sandbox.ubuntu.UbuntuPaths.resolveHostPath(linuxPath)

    fun resolveSessionHostPath(sessionId: String, linuxPath: String, context: Context): File? =
        com.openminis.app.sandbox.ubuntu.UbuntuPaths.resolveSessionHostPath(sessionId, linuxPath, context)
