...
import com.openminis.app.tools.android.UiActionEvidenceSource
import com.openminis.app.tools.android.UiChangeObservation
...
"""
    ...
    private fun inputText(args: OffloadArgs): NativeOffloadResult {
        ...
        val accepted = svc.setNodeText(node, finalText)
        val verified = if (accepted) svc.verifyNodeText(node, finalText) else null
        val report = AndroidUiActionEvidence.ofTextInput(accepted, verified)
        val data = report.into(JSONObject().put("text", finalText).put("action", "setText")
            .put("verified", verified ?: JSONObject.NULL))
        return if (accepted) ok(args, data)
        else err(args, "ACTION_FAILED", "ACTION_SET_TEXT failed (node may not be editable)", report = report)
    }
    ...
    private fun inputClear(args: OffloadArgs): NativeOffloadResult {
        ...
        val node = resolveTargetEditable(svc, args)
            ?: return err(args, "NODE_NOT_FOUND", "no editable focus and no --node specified")
        val accepted = svc.setNodeText(node, "")
        val verified = if (accepted) svc.verifyNodeText(node, "") else null
        val report = AndroidUiActionEvidence.ofTextInput(accepted, verified)
        val data = report.into(JSONObject().put("action", "clear").put("verified", verified ?: JSONObject.NULL))
        return if (accepted) ok(args, data)
        else err(args, "ACTION_FAILED", "ACTION_SET_TEXT('') failed", report = report)
    }
    ...
    private fun inputKey(args: OffloadArgs): NativeOffloadResult {
        ...
        // without returning may have run, so it is not reported as a plain failure.
        val report = AndroidUiActionEvidence.ofGlobalAction(
            svc.performGlobalActionGated(action),
            UiChangeObservation.UNKNOWN,
        )
        val data = report.into(JSONObject().put("key", keyName.uppercase()))
        if (report.evidence.acceptedBySystem) return ok(args, data)
        return err(
            args,
            when (report.source) {
                UiActionEvidenceSource.MAIN_THREAD_NOT_DISPATCHED -> "ACTION_NOT_DISPATCHED"
                UiActionEvidenceSource.MAIN_THREAD_TIMEOUT -> "ACTION_TIMEOUT"
                else -> "ACTION_FAILED"
            },
            when (report.source) {
                UiActionEvidenceSource.MAIN_THREAD_NOT_DISPATCHED ->
                    "${keyName.uppercase()} was cancelled before it started; the system never ran it"
                UiActionEvidenceSource.MAIN_THREAD_TIMEOUT ->
                    "${keyName.uppercase()} did not return before the deadline; it may still be in flight"
                else -> "the system refused ${keyName.uppercase()}"
            },
            report = report,
        )
    }
    ...
    private fun gesturePinch(args: OffloadArgs): NativeOffloadResult {
        ...
        val duration = args.getLong("duration") ?: 300L
        val report = svc.dispatchPinchWithEvidence(cx, cy, scale, duration)
        val data = report.into(JSONObject().put("center", JSONArray().put(cx).put(cy)).put("scale", scale))
        if (report.evidence.acceptedBySystem) return ok(args, data)
        return err(
            args,
            when (report.source) {
                UiActionEvidenceSource.GESTURE_NOT_DISPATCHED -> "GESTURE_NOT_DISPATCHED"
                UiActionEvidenceSource.GESTURE_CANCELLED -> "GESTURE_CANCELLED"
                UiActionEvidenceSource.GESTURE_TIMEOUT -> "GESTURE_TIMEOUT"
                else -> "GESTURE_FAILED"
            },
            when (report.source) {
                UiActionEvidenceSource.GESTURE_NOT_DISPATCHED ->
                    "pinch was cancelled before it started; the system never dispatched it"
                UiActionEvidenceSource.GESTURE_CANCELLED -> "pinch was dispatched and the system cancelled it"
                UiActionEvidenceSource.GESTURE_TIMEOUT ->
                    "pinch did not finish before the deadline; it may still be in flight"
                else -> "pinch was not accepted"
            },
            report = report,
        )
    }
    ...
    private fun err(
        ...
        message: String,
        exit: Int = 1,
        report: UiActionEvidenceReport? = null,
    ): NativeOffloadResult {
        val error = JSONObject().put("code", code).put("message", message)
        report?.into(error)
        val body = JSONObject().put("ok", false).put("error", error).toString()
