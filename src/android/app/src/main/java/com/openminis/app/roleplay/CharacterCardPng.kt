...
/**
 * [T-eta-character-cards] The character card hidden inside a PNG: two `tEXt` chunks named
 * `chara` (V2) and `ccv3` (V3), Base64 over the JSON body.
 *
 * Ported from Eta `agent/roleplay/CharacterCardPng.kt` (Mangi-11/Eta @ c15de97); attribution in
 * THIRD_PARTY_LICENSES.md. Every rule is Eta's: the chunks are validated (length, CRC, IHDR
 * geometry, colour type and depth, IDAT present) before a single byte is trusted, V3 wins over
 * V2 when both exist, and a corrupt V3 is reported instead of silently falling back to the older
 * copy. Writing strips the old card chunks and inserts fresh ones before IEND.
 */
object CharacterCardPng {
    ...
    fun read(bytes: ByteArray): CharacterCard {
        val text = chunks(bytes).filter { it.type == "tEXt" }.mapNotNull { chunk ->
            ...
            if (separator <= 0) {
                ...
                chunk.data.copyOfRange(0, separator).toString(Charsets.ISO_8859_1).lowercase() to
                    chunk.data.copyOfRange(separator + 1, chunk.data.size)
            }
        }
        val v3 = text.firstOrNull { it.first == "ccv3" }?.second
        val encoded = v3 ?: text.firstOrNull { it.first == "chara" }?.second
        ...
        return try {
            ...
            if (v3 != null) {
                throw CharacterCardException("CARD_V3_INVALID", "PNG 中的 V3 角色数据损坏，未使用旧版本数据替代", failure)
            }
    ...
    fun write(image: ByteArray, card: CharacterCard): ByteArray {
        ...
        DataOutputStream(output).use { stream ->
            original.forEach { chunk ->
                val keyword = if (chunk.type == "tEXt") {
                    chunk.data.takeWhile { it != 0.toByte() }.toByteArray()
                ...
                if (chunk.type == "IEND") {
                    listOf("chara" to 2, "ccv3" to 3).forEach { (key, version) ->
                        val encoded = Base64.getEncoder().encode(
                            CharacterCardCodec.exportView(card, version).toByteArray(Charsets.UTF_8),
                        )
        ...
        if (output.size() > MAX_FILE_BYTES) {
            throw CharacterCardException("CARD_TOO_LARGE", "PNG 角色卡超过 32 MiB，请改用 JSON 导出")
        }
    ...
    private fun chunks(bytes: ByteArray): List<Chunk> {
        ...
        require(width > 0 && height > 0 && width.toLong() * height <= 64_000_000L) { "PNG 图片尺寸过大或无效" }
        ...
        require(
            bitDepth in allowedDepths && header[10] == 0.toByte() && header[11] == 0.toByte() &&
                header[12].toInt() in 0..1,
        ) { "PNG 图片头格式无效" }
