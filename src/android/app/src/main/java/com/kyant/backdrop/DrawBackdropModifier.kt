...
// A LayerBackdrop cannot record a descendant that consumes that same layer:
// layer -> consumer -> layer is an upstream-documented RenderThread SIGSEGV.
// This outer draw node makes the consumer a logical sibling during recording,
// while leaving its normal on-screen draw untouched.
private fun Modifier.avoidBackdropSelfSampling(backdrop: Backdrop): Modifier =
    if (backdrop is LayerBackdrop) {
        drawWithContent {
            if (!backdrop.isRecordingContent) drawContent()
        }
    } else {
        this
    }
...
fun Modifier.drawPlainBackdrop(
    ...
    return this
        .avoidBackdropSelfSampling(backdrop)
        .then(
            if (layerBlock != null) {
