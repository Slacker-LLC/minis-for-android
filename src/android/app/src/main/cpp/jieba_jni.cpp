...
Java_io_github_slackerllc_minis_shared_JiebaEngine_nativeInit(
...
Java_io_github_slackerllc_minis_shared_JiebaEngine_nativeSegment(
...
JNIEXPORT jobjectArray JNICALL
Java_io_github_slackerllc_minis_shared_JiebaEngine_nativeSegmentForSearch(
    JNIEnv* env, jobject /* thiz */, jstring text) {
    std::lock_guard<std::mutex> lock(g_mutex);
    if (!g_state) {
...
// JiebaState.
JNIEXPORT jobjectArray JNICALL
Java_io_github_slackerllc_minis_shared_JiebaEngine_nativePosTag(
    JNIEnv* env, jobject /* thiz */, jstring text) {
    std::lock_guard<std::mutex> lock(g_mutex);
    if (!g_state) {
