...
fn root_access_mode(req: &Request) -> Result<&str, ErrorCode> {
    match req.params.get("access_mode").and_then(Value::as_str).unwrap_or("standard") {
...
pub fn authorize_request(
    ...
    if req.method != "system.hello" {
        ...
        return Err(Response::err(req.id, ErrorCode::NotAuthorized, "client.id required"));
    }
        ...
        match req.confirm_id.as_deref() {
            ...
            Some(cid) => {
                if let Err(code) = state.consume_confirm(cid, &key) {
                    return Err(Response::err(req.id, code, "invalid, mismatched or reused confirm_id"));
                }
            }
        }
    }
    if let Some(limit) = decision.rate_per_min {
        if !state.rates.check(&req.method, limit, state.now()) {
            return Err(Response::err(req.id, ErrorCode::RateLimited, "ratePerMin exceeded"));
        }
    }
    Ok(())
}
...
pub fn handle(state: &mut AppState, req: Request, peer: Option<PeerCred>) -> Response {
    if let Err(resp) = authorize_request(state, &req, peer) { return resp; }
...
fn requested_runtime_layout(params: &Value) -> (String, String, String, String) {
    fn path_or(params: &Value, key: &str, fallback: &str) -> String {
        params.get(key).and_then(Value::as_str).filter(|s| !s.is_empty()).unwrap_or(fallback).to_string()
    }
    (
...
fn runtime_layout_known(state: &AppState) -> bool {
    !state.ubuntu.workspace.is_empty() && !state.ubuntu.memory.is_empty()
        && !state.ubuntu.skills.is_empty() && !state.ubuntu.shared.is_empty()
}
...
fn start_ubuntu_with_layout(state: &mut AppState, req: &Request) -> Response {
    ...
    if state.ubuntu.running {
        if !runtime_layout_known(state) {
            return Response::err(req.id, ErrorCode::RuntimeLayoutMismatch, "running keeper bind layout is unknown; stop and restart required");
        }
        if state.ubuntu.workspace != workspace || state.ubuntu.memory != memory
            || state.ubuntu.skills != skills || state.ubuntu.shared != shared
        {
            return Response::err(req.id, ErrorCode::RuntimeLayoutMismatch, "requested host bind paths differ from the running keeper");
        }
    }
...
fn ubuntu_status_with_layout(state: &mut AppState) -> Value {
    ...
    if let Some(obj) = value.as_object_mut() {
        ...
        obj.insert("workspace".into(), if known { Value::String(state.ubuntu.workspace.clone()) } else { Value::Null });
        obj.insert("memory".into(), if known { Value::String(state.ubuntu.memory.clone()) } else { Value::Null });
        obj.insert("skills".into(), if known { Value::String(state.ubuntu.skills.clone()) } else { Value::Null });
        obj.insert("shared".into(), if known { Value::String(state.ubuntu.shared.clone()) } else { Value::Null });
    }
    value
}
...
pub fn dispatch_authorized(state: &mut AppState, req: &Request) -> Response {
    match req.method.as_str() {
        ...
        "root.exec" => execute_root_authorized(state.mock, state.policy.method("root.exec").cloned(), req),
        ...
        "ubuntu.stop" => match crate::ubuntu::stop(state) {
            Ok(v) => {
                state.ubuntu.workspace.clear(); state.ubuntu.memory.clear();
                state.ubuntu.skills.clear(); state.ubuntu.shared.clear();
                Response::ok(req.id, v)
            }
            Err((code, detail)) => Response::err(req.id, code, detail),
        },
        "ubuntu.exec" => match crate::ubuntu::exec(state, &req.params, false) {
            Ok(v) => Response::ok(req.id, v), Err((code, detail)) => Response::err(req.id, code, detail),
        },
        ...
        "mount.list" => {
            if !state.mock { return Response::err(req.id, ErrorCode::RuntimeUnavailable, "mount ns not started"); }
            Response::ok(req.id, json!({"mounts": []}))
        }
        "mount.prepare" => {
            if state.mock { Response::ok(req.id, json!({"prepared": true, "via": "mock"})) }
            else { Response::err(req.id, ErrorCode::RuntimeUnavailable, "mount ns not started") }
        }
        "workspace.info" => Response::ok(req.id, json!({
            "path": if runtime_layout_known(state) { Some(state.ubuntu.workspace.clone()) } else { None::<String> },
            "guest": "/workspace", "layout_known": runtime_layout_known(state),
            "quota_bytes": state.workspace_quota_bytes
        })),
        "workspace.setQuota" => {
            let n = req.params.get("quota_bytes").and_then(|v| v.as_u64());
            let Some(n) = n else { return Response::err(req.id, ErrorCode::BadParams, "quota_bytes required"); };
            state.workspace_quota_bytes = n;
            Response::ok(req.id, json!({"quota_bytes": n}))
        }
        "policy.get" => Response::ok(req.id, json!({
        ...
        "health.get" => {
            ...
            Response::ok(req.id, json!({
                "ok": rootfs_ok && (!state.ubuntu.running || layout_known), "mock": state.mock,
                "ubuntu": state.ubuntu.running, "ubuntu_pid": state.ubuntu.pid,
                "ubuntu_version": state.ubuntu.version, "rootfs_ok": rootfs_ok,
                "layout_known": layout_known, "sessions": state.sessions.len(), "selinux_enforcing": true
            }))
        }
...
pub fn execute_root_authorized(mock: bool, spec: Option<MethodPolicy>, req: &Request) -> Response {
    let parsed = match parse_exec(&req.params) {
        Ok(p) => p, Err(code) => return Response::err(req.id, code, "bad exec params"),
    };
    let access_mode = match root_access_mode(req) {
        Ok(mode) => mode, Err(code) => return Response::err(req.id, code, "invalid root access mode"),
    };
    ...
    if mock {
        ...
        return Response::ok(req.id, json!({
            "exit_code": 0, "stdout_bytes": stdout.len(), "stderr_bytes": 0,
            "stdout_truncated": false, "stderr_truncated": false,
            "stdout": stdout, "stderr": "", "access_mode": access_mode
        }));
    }
    let dynamic_allow;
    ...
    match run_exec(&parsed, &allow) {
        Ok(out) => Response::ok(req.id, json!({
            "exit_code": out.exit_code, "stdout": out.stdout, "stderr": out.stderr,
            "stdout_bytes": out.stdout_bytes, "stderr_bytes": out.stderr_bytes,
            "stdout_truncated": out.stdout_truncated, "stderr_truncated": out.stderr_truncated,
...
fn kill_tree(state: &mut AppState, req: &Request) -> Response {
    ...
    if session.is_empty() { return Response::err(req.id, ErrorCode::BadParams, "session required"); }
    if state.mock {
        if state.sessions.get(session).is_none() {
            state.sessions.insert(Session { id: session.to_string(), pgid: 1000, children: vec![1001] });
        }
        let mut killer = RecordingKiller { term_alive: false, calls: vec![] };
        return match state.sessions.kill_tree(session, std::time::Duration::from_millis(10), &mut killer) {
    ...
    #[cfg(unix)]
    {
        ...
        match state.sessions.kill_tree(session, std::time::Duration::from_secs(2), &mut killer) {
            Ok(steps) => {
                let _ = sess;
                Response::ok(req.id, json!({
                    "steps": steps.len(),
                    "pgid": steps.first().map(|s| match s {
                        crate::session::KillStep::Term { pgid } | crate::session::KillStep::Kill { pgid } => *pgid,
                    })
                }))
            }
            Err(code) => Response::err(req.id, code, "killTree failed"),
...
mod tests {
    ...
    fn hello(method: &str) -> ClientHello {
        ClientHello { id: "app".into(), capabilities: vec![method.into()], token: None }
    }
    fn req(method: &str, params: serde_json::Value) -> Request {
        Request { v: 1, id: 1, method: method.into(), client: Some(hello(method)), params, confirm_id: None }
    }
    ...
    fn standard_allowlisted_root_exec_runs_without_confirmation() {
        let mut state = AppState::new(true, PolicyFile::default_policy());
        let request = req("root.exec", json!({"tool":"getprop","args":[],"access_mode":"standard"}));
        assert!(authorize_request(&mut state, &request, None).is_ok());
        assert!(dispatch_authorized(&mut state, &request).ok);
    }
    ...
    fn standard_outside_allowlist_requires_full_request_bound_confirmation() {
        let mut state = AppState::new(true, PolicyFile::default_policy());
        let first = req("root.exec", json!({"tool":"debuggerd","args":["123"],"access_mode":"standard"}));
        let cid = authorize_request(&mut state, &first, None)
            .unwrap_err().error.unwrap().confirm_id.unwrap();

        let mut changed = req("root.exec", json!({"tool":"debuggerd","args":["456"],"access_mode":"standard"}));
    ...
    fn standard_confirmation_is_one_shot_and_executes_exact_request() {
        let mut state = AppState::new(true, PolicyFile::default_policy());
        let mut request = req("root.exec", json!({"tool":"cmd","args":["package","list","packages"],"access_mode":"standard"}));
        let cid = authorize_request(&mut state, &request, None)
            .unwrap_err().error.unwrap().confirm_id.unwrap();
        request.confirm_id = Some(cid.clone());
        assert!(authorize_request(&mut state, &request, None).is_ok());
        assert!(execute_root_authorized(true, state.policy.method("root.exec").cloned(), &request).ok);
        let mut replay = request.clone();
        replay.confirm_id = Some(cid);
        assert_eq!(authorize_request(&mut state, &replay, None).unwrap_err().error.unwrap().code, "POLICY_DENIED");
    }

    #[test]
    fn full_access_bypasses_standard_allowlist_without_confirmation() {
        let mut state = AppState::new(true, PolicyFile::default_policy());
        let request = req("root.exec", json!({"tool":"sh","args":["-c","id"],"access_mode":"full"}));
        assert!(authorize_request(&mut state, &request, None).is_ok());
        assert!(execute_root_authorized(true, state.policy.method("root.exec").cloned(), &request).ok);
    }
    ...
    fn confirm_once_and_reuse_rejected() {
        ...
        assert!(handle(&mut state, req("ubuntu.start", json!({})), None).ok);
        assert!(handle(&mut state, second, None).ok);
        let mut third = req("ubuntu.adminExec", json!({"argv":["/usr/bin/apt-get","update"]}));
        third.confirm_id = Some(cid);
        assert_eq!(handle(&mut state, third, None).error.unwrap().code, "POLICY_DENIED");
    }
    ...
    fn parse_and_handle_ping() {
        ...
        assert!(handle(&mut state, parse_request(raw).unwrap(), None).ok);
    }
}
