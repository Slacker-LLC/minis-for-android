...
### 设置界面统一：一套组件、一个主题一页（2026-09-19，真机测量 + 重构） — `d93682d5`

| 项 | 结果 |
|---|---|
| 怎么发现的 | 用 `uiautomator dump` 量了主设置页与子页（`wm density` = 520dpi，1dp = 3.25px）：主设置页行高是 **44.6 / 64.9 / 72.0 / 80.9dp**（内容驱动，单行行比子页矮），子页是 **56dp**（选择行）与 **64.9dp**（两行行）；主设置页标题缩进 20dp、卡片圆角 12dp、行内边距 14dp，子页是 32 / 14 / 16dp；分隔线有 14 / 16 / 38 / 58dp 四种起点 |
| 根因 | 主设置页私有一套 `SettingsSection/SettingsItem/SettingsToggleItem`（928 行文件里约 200 行是重复实现，圆角 12、圆形象素图标、无最小行高），与共享的 `SettingsComponents` 并存且数值不同；全目录统计：9 种圆角、5 种水平内边距、2 种屏底留白 |
| 改了什么 | ① 新增 `SettingsMetrics`（卡片边距 16、行内边距 16/12、行高 56、图标块 30/圆角 8、分隔线缩进、分区圆角 14、分区间距 24、屏底 24）；② 共享组件全部走它，并新增唯一的 `SettingsDivider`（替换四种写法）；③ 主设置页改用共享组件，删掉私有三件套；④ 屏底留白由 `SettingsScaffold` 统一提供 |
| 信息架构 | 主设置页由 9 个标题（含 2 个无标题分区）并成 6 组：**模型与用量 / 助手能力 / 运行时与沙箱 / 外观 / 系统与权限 / 诊断与关于**；「模块设置」与「系统增强」讲的是同一件事，开关并入「系统增强」，多余的路由、页面与入口行一并删除 |
| 顺手修的 i18n | 「角色」两行在中文界面里显示英文——`characters_title/subtitle` 在 **8 个语言文件里都是英文原文**；另有 4 处硬编码中文（自动压缩上下文、桌面宠物、子代理委派限制、默认数字助手）进入资源，共新增 16 个键 × 8 语言 |
| 真机验证 | 主设置页行高统一（两行 64.9dp；开关行与普通行同高；选择行 56dp），图标块统一为圆角方形，分隔线对齐；系统增强页一屏包含 状态 → 手势开关（含未连接提示）→ 电源键助手（含默认助手状态）→ 能力说明 |
| 顺带结论（孤儿页） | 全项目扫描 65 个 `*Screen` 组合函数：只有 `ui/onboarding/OnboardingScreen.kt` **从未被任何地方引用**（导航图里没有它，深链接也没有），属死代码；是否删除待用户点头。其余设置页都有入口 |
...
### 工作过程行改成 Codex 形态（纯文字 + 箭头 + 细线）与时长持久化（2026-09-19，真机） — `56d91437`

| 项 | 结果 |
|---|---|
| 用户反馈 | 「工具框和 AI 消息两端不对齐，框短、文字长，看着难受」；附 codex 两张局部图：运行/收起都是**一行灰字 + 箭头 + 下方细线**，没有卡片框 |
| 改法 ①（形态） | `WorkProcessRowView` 去掉圆角卡片（`clip(RoundedCornerShape(12)) + background + border + 12dp 内边距`），改成：一行文字（左对齐、占满消息列宽）+ 右侧箭头（**收起 `>` / 展开 `V`**，与 codex 一致）+ 下方 0.5dp 细分割线；行内容与回答文字同一左边缘（实测 x=52，与消息文本一致） |
| 改法 ②（时长持久化） | 之前时长只在"这一轮还在内存里"时存在——重载后回退成"已完成 N 步"，正是用户说的「跑完就没有用时了」。现在结束标记用**持久化列**：每条行贡献 `updated_at`（为空则用它的 `created_at`，因为只写一次的行没有 updated_at），合并本轮时取二者**较晚**的那个；起点仍是用户消息的 `created_at`。重启后实测：恢复出来的行显示 `用时 10s · 第 3 步失败` |
...
### 外观新增「强调色」（2026-09-19，真机验证） — `ece26210`
...
### 中文文案清零 + 聊天/设置/MCP 界面文案进资源（2026-09-19，真机投诉驱动） — `2e2ff656`

| 项 | 结果 |
|---|---|
| 用户反馈 | ①「我现在中文模式，居然还有这么多英文，混乱得很」②「MCP 集成那一块还是英文」 |
| 审计（可复跑） | `/tmp/minis-i18n-audit/audit_strings.py --repo <仓库> --out <目录>`：把 8 个语言目录逐 key 与默认英文对比，产出六类清单（同英文 / 缺失 / 孤儿 / 复数缺口 / 格式参数错配 / Kotlin 硬编码字面量），本次施工按清单进行 |
| 简体中文 | `values-zh` 由 2044 补到 **2110 key，覆盖 100%**：补 66 条缺失（语音服务、系统权限、无障碍说明、文件浏览器、模型快速测试、MCP 导出等），改写 44 条照抄英文（角色卡 25、提供商 4、Shizuku 能力名 12、温度 / API Key / 导入提供商） |
| 硬编码进资源 | 30 个 Kotlin 文件：聊天顶栏与输入区、工具卡片、压缩摘要弹窗、消息操作、回复语音条；提供商 / 技能 / 存储 / 模型组 / 环境变量页与确认弹窗；文件与媒体预览、Markdown 渲染、浏览器设置；**MCP 暴露区块（Expose Minis、服务器状态行、访问令牌、生成 / 轮换、工具范围弹窗、复制连接配置、撤销令牌）** |
| 新增 key | **117 条**，一律 8 语言同步（英文默认 + 简体 / 日 / 韩 / 德 / 法 / 俄）；繁體中文留到后续转换批次 |
| 明确不改 | 品牌与协议名（Minis / Shizuku / MCP / HTTP / SSE / STDIO / JSON / Token / Responses API / Claude Desktop）、slash 命令标题（`/model` 等是选择器插入的命令 token，不是展示文案）、模型侧提示词正文与日志 |
...
| 繁體同步 | 新增的 52 个聊天外壳 key 让繁體再次落后，已按同一流程（OpenCC s2twp，从简体原文转换）补齐：zh-rTW 2297/2297，0 条回退英文 |
