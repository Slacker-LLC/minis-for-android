...
状态：历史 RFC 参考，不是当前实现清单。`main` 已有 `backup/` 中的导出、导入、加密和 rclone 相关代码；本文保留设计风险与验收思路，不表示这些功能尚未开发，也不表示已完成真机验收。当前实现以源码、测试及[存储合同](../contracts/03-STORAGE-CONTRACT.md)为准。
...
`.minisbak` 的原始设计目标是跨设备、可校验、可加密的单文件包格式。下文实施阶段是历史规划；现在应对照已有实现补齐单元、集成和真机验收，不应重复另建一套备份系统。
...
| 包格式/加密边界 | 历史设计 | 已有 `BackupFormat` / `BackupCrypto` 等代码；安全性质以测试为准 |
| App-owned 一致性导出 | 需明确 snapshot seam | 已有 `BackupExporter`；本文不证明并发一致性验收通过 |
| 崩溃/断电可逆恢复 | 历史恢复目标 | 已有 `BackupImporter`；本文不证明断电回滚矩阵通过 |
| rclone transport | 历史集成边界 | 已有相关实现；远程上传/恢复仍需独立功能验收 |
| 真机权限/恢复 smoke test | 必须验证 | 本轮未重新执行 |

## 9. 本 RFC 的交付边界

本文不改变 Android 代码、存储路径、APK、远程凭据或设备状态。它只把 #122 的 feasibility 边界与当前 direct Ubuntu/App-owned storage 对齐。

后续应检查已有实现与验收缺口，不重复建立备份系统；不能把“RFC 完成”描述成“备份功能已全面验收”。
